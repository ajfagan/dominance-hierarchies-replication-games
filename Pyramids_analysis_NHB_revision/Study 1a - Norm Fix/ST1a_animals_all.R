...
# norm_factor<-apply(dom_mat_z_score[1:16,]^2,1,function(x) sqrt(sum(x,na.rm=T))) # calculate sqrt of SS
# dom_mat_signif_prof<-dom_mat_z_score[1:16,] * (1/norm_factor)
...
dom_mat_signif_prof_df[c("X021D","X021U","X021C","X030T","X030C")] =
  lapply(dom_mat_signif_prof_df[c("X021D","X021U","X021C","X030T","X030C")],
         function(x){x[is.nan(x)] = 0; return(x)})
dom_mat_signif_prof_df[c("ttm","tpm")] =
  lapply(dom_mat_signif_prof_df[c("ttm","tpm")],
         function(x){x[is.nan(x)] = 0.5; return(x)})
...
dom_mat_SP_plot
ggsave(dom_mat_SP_plot, filename="dom_mat_SP_plot_all.jpeg",width = 15, height=30, units = "cm")

# Save outcomes
dom_mat_SP_df_animals<-dom_mat_SP_df
dom_mat_SP_df_long_animals<-dom_mat_SP_df_long
save(dom_mat_SP_df_long_animals,file="dom_mat_SP_df_long_animals_all.Rdata")
save(dom_mat_SP_df_animals,file="dom_mat_SP_df_animals_all.Rdata")
